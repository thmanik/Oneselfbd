...
const TimelineEvent: React.FC<TimelineEventProps> = ({
  ...
  return (
    <div className={`relative flex mb-12 ${isLeft ? "flex-row-reverse" : ""}`}>
      {/* Vertical line */}
      <div
        className={`absolute w-1 ${isActive ? "bg-primary" : "bg-gray-300"} top-0 bottom-0 left-1/2 transform -translate-x-1/2`}
      ></div>
      ...
      {!isLast && (
        <div
          className={`absolute w-1 ${isActive ? "bg-primary" : "bg-gray-300"} left-1/2 transform -translate-x-1/2 top-full`}
